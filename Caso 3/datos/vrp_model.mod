...
# Nuevos parámetros para peajes y restricciones de peso
param toll_base_rate {N} >= 0;   # Tarifa base del peaje en cada nodo
param toll_rate_per_ton {N} >= 0; # Tarifa adicional por tonelada en cada peaje
param weight {N} >= 0;            # Peso de la carga en cada municipio (toneladas)
...
# ========================
minimize TotalCost:
    # Costos de distancia (flete y mantenimiento)
    sum {v in V, i in N, j in N : i != j} (Ft + Cm) * dist[i,j] * x[v,i,j]
    # Costos de combustible
    + sum {v in V, i in N} fuel_price[i] * r[v,i]
    # Costos de peaje: tarifa base
    + sum {v in V, i in N, j in D : i != j} toll_base_rate[j] * x[v,i,j]
    # Costos de peaje: tarifa adicional por tonelada
    + sum {v in V, i in N, j in D : i != j} toll_rate_per_ton[j] * weight[j] * x[v,i,j];
...
# Conjunto para nodos que deben pasar por estaciones específicas
...
# ========================
# Nuevas restricciones para pesos por municipio
# ========================

# Restricción opcional: Límite de peso por vehículo basado en capacidad del vehículo
subject to WeightCapacity {v in V}:
    sum {j in D} weight[j] * sum {i in N : i != j} x[v,i,j] <= Q[v] * z[v];